  ...
  <div class="flex items-center gap-2">
    <div class="flex items-center gap-2 text-primary-50">
      ...
      <button
        ...
        class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-secondary-500 focus:ring-offset-2 border-2 border-primary-300 {easyMode
          ? 'bg-secondary-600'
          : 'bg-primary-800'}"
      >
        <span
          class="inline-block h-4 w-4 transform rounded-full bg-primary-50 shadow-md transition-transform {easyMode
            ? 'translate-x-[1.35rem]'
            : 'translate-x-0.5'}"
        ></span>
      </button>
